...
void CDiscordRPC::Update()
{
	...
	if (!Vars::Misc::Paste::EnableRPC.Value)
	{
		if (m_IsActive)
		{
			Discord::ClearPresence();
	...
	{
		if (Vars::Misc::Paste::IncludeMap.Value)
		{
			char mapName[256];
		...
		{
			if (Vars::Misc::Paste::IncludeClass.Value)
			{
				if (pLocal->IsAlive())
				{
	...
	// Timestampt
	if (Vars::Misc::Paste::IncludeTimestamp.Value)
	{
