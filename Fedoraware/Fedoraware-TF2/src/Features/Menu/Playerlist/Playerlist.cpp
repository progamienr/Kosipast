			...
			if (pr && pr->GetValid(i) && pr->GetConnected(i))
			{
				...
				if (I::EngineClient->GetPlayerInfo(i, &info))
				{
					ListPlayer player{
						...
						pr->GetUserID(i),
						pr->GetAccountID(i),
						GetTeamColor(pr->GetTeam(i), Vars::ESP::Main::EnableTeamEnemyColors.Value),
						pr->GetHealth(i),
						pr->GetMaxHealth(i),
						pr->GetClass(i),
						pr->IsAlive(i),
...
void CPlayerList::Render()
{
	...
	if (ImGui::Begin("Playerlist", &Vars::Menu::ShowPlayerlist.Value, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse))
	{
