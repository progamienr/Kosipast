...

	{
		...
		if (chamsMaterial == v_MatList.at(7))
		{
			if (IMaterialVar* $envmaptint = chamsMaterial->FindVar("$envmaptint", nullptr, false))
			{
				$envmaptint->SetVecValue(
					Color::TOFLOAT(chams.Color.r) * 4,
					Color::TOFLOAT(chams.Color.g) * 4,
					Color::TOFLOAT(chams.Color.b) * 4);
			}
		...
		else
		{
			I::RenderView->SetColorModulation(
				Color::TOFLOAT(rainbow ? Utils::Rainbow().r : chams.Color.r),
				Color::TOFLOAT(rainbow ? Utils::Rainbow().g : chams.Color.g),
				Color::TOFLOAT(rainbow ? Utils::Rainbow().b : chams.Color.b));
		}

		I::RenderView->SetBlend(chams.Color.a);
			...
			if (pMaterial)
			{
				if (IMaterialVar* $phongtint = pMaterial->FindVar("$phongtint", nullptr, false))
				{
					$phongtint->SetVecValue(
						Color::TOFLOAT(rainbowOverlay ? Utils::Rainbow().r : chams.OverlayColor.r),
						Color::TOFLOAT(rainbowOverlay ? Utils::Rainbow().g : chams.OverlayColor.g),
						Color::TOFLOAT(rainbowOverlay ? Utils::Rainbow().b : chams.OverlayColor.b));
				}
				...
				I::RenderView->SetBlend(chams.OverlayPulse ? sin(I::GlobalVars->curtime * 5) * 0.5f + 0.51f : Color::TOFLOAT(chams.OverlayColor.a));
...
bool CDMEChams::Render(const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo, matrix3x4* pBoneToWorld)
{
	...
	if (ShouldRun())
	{
		...
		// handle
		{
			...
			float alpha = Color::TOFLOAT(chams.Color.a);
			if (pEntity && pLocal)
			{
