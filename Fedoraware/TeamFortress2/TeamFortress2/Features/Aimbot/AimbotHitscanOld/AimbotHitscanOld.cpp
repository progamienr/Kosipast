...
bool CAimbotHitscan::VerifyTarget(CBaseEntity* pLocal, Target_t& target)
{
	...
	{
		if (F::AimbotHitscan.ScanHitboxes(pLocal, target) && !(!F::Backtrack.CanHitOriginal(target.m_pEntity) || G::ChokeMap[target.m_pEntity->GetIndex()] > Vars::Aimbot::Global::TickTolerance.Value || Vars::Backtrack::Method.Value == 4))
		{
			return true;
		}
		if (Vars::Backtrack::Enabled.Value)
		{
			...
			{
				if (!IsHitboxValid(nHitbox, Vars::Aimbot::Hitscan::ScanHitboxes.Value, (target.m_pEntity->GetVelocity().Length() < 10.f))) { continue; }
				if (std::optional<TickRecord> ValidRecord = F::Backtrack.Aimbot(target.m_pEntity, (BacktrackMode)Vars::Backtrack::Method.Value, nHitbox))
				{
					target.pTick = &ValidRecord.value();
					target.m_vPos = target.m_pEntity->GetHitboxPosMatrix(nHitbox, (matrix3x4*)(&ValidRecord->BoneMatrix.BoneMatrix));
					target.m_vAngleTo = Math::CalcAngle(pLocal->GetShootPos(), target.m_vPos);
		...
		if (bIsAttacking)
		{
			...
			if (target.pTick)
			{
				...
				if (Vars::Aimbot::Global::ShowHitboxes.Value)
				{
					...
					F::Visuals.DrawHitbox((matrix3x4*)(&(*target.pTick).BoneMatrix.BoneMatrix), set);
				}
			}
		}

		// Set the players tickcount (Backtrack / Interpolation removal)
		const float simTime = target.ShouldBacktrack ? (*target.pTick).flSimTime : target.m_pEntity->GetSimulationTime();
