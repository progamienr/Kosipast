...
bool CAimbotProjectile::SolveProjectile(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, CUserCmd* pCmd, Predictor_t& predictor, const ProjectileInfo_t& projInfo, Solution_t& out)
{
	...
	{
		cl_flipviewmodels->m_Value.m_nValue = oValue;
	}
	...
	const float fLatency = F::Backtrack.GetReal();//pNetChannel->GetLatency(FLOW_OUTGOING) + pNetChannel->GetLatency(FLOW_INCOMING) + G::LerpTime;	//	correct
	...
	*/
	...
	else
	{
		...
		if (F::MoveSim.Initialize(predictor.m_pEntity, storage))
			return false;
		...
		for (int n = 0; n < TIME_TO_TICKS(maxTime); n++)
		{
			...
			vPredictedPos = storage.m_MoveData.m_vecAbsOrigin;
			...
			if (out.m_flTime < TICKS_TO_TIME(n))
			{
				...
				{
					G::PredictionLines = storage.PredictionLines;
					G::PredictedPos = vPredictedPos;
					F::MoveSim.Restore(storage);
	...
	if (GetTarget(pLocal, pWeapon, pCmd, target) || GetSplashTarget(pLocal, pWeapon, pCmd, target))
	{
		...
		if (bIsAttacking)
		{
			...
			if (Vars::Visuals::SimLine.Value)
				G::LinesStorage.push_back({ G::PredictionLines, I::GlobalVars->curtime, Vars::Aimbot::Projectile::PredictionColor });
