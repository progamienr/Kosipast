...
{
	if (!bSaved)
		{ vSavedAngles = I::EngineClient->GetViewAngles(); bSaved = true; }
	const short dMouseX = pCmd->mousedx;
	const short dMouseY = pCmd->mousedy;
	if (abs(dMouseX) > abs(dMouseY))
	{
		if (dMouseX > 25)
			p_p_bManualYaw = { false, {false, false} };	//right
		else if (dMouseX < -25)
			p_p_bManualYaw = { false, {true, false} };	//left
	}
	else
	{
		if (dMouseY > 25)
			p_p_bManualYaw = { true, {false, true} };	//down
		else if (dMouseY < -25)
			p_p_bManualYaw = { true, {false, false} };	//up
	}
...
void CAntiAim::FakeShotAngles(CUserCmd* pCmd){
	if (!G::IsAttacking || G::CurWeaponType != EWeaponType::HITSCAN || !Vars::AntiHack::AntiAim::InvalidShootPitch.Value)
		return;
...
float CAntiAim::YawIndex(int iIndex)
{
	...
	switch (iIndex)
	{
		...
		case 11: {
			return !bSendState
				? (p_bJitter.first ? Vars::AntiHack::AntiAim::RealJitter.Value : 0.f)
				: (p_bJitter.second ? Vars::AntiHack::AntiAim::FakeJitter.Value : 0.f);
		}
		case 12: {
			return !bSendState
				? (p_bJitter.first
					? Utils::RandFloatRange(fminf(Vars::AntiHack::AntiAim::RealJitter.Value, 0.f), fmaxf(Vars::AntiHack::AntiAim::RealJitter.Value, 0.f))
					: 0.f)
				: (p_bJitter.second
					? Utils::RandFloatRange(fminf(Vars::AntiHack::AntiAim::FakeJitter.Value, 0.f), fmaxf(Vars::AntiHack::AntiAim::FakeJitter.Value, 0.f))
					: 0.f);
		}
		case 13: {
			return !bSendState
				? (p_bJitter.first
					? Vars::AntiHack::AntiAim::RealJitter.Value
					: -Vars::AntiHack::AntiAim::RealJitter.Value)
				: (p_bJitter.second
					? Vars::AntiHack::AntiAim::FakeJitter.Value
					: -Vars::AntiHack::AntiAim::FakeJitter.Value);
		}
		...
		case 2:
		{
			...
			for (CBaseEntity* pEnemy : g_EntityCache.GetGroup(EGroupType::PLAYERS_ENEMIES))
			{
				...
				if (flFOVTo < flSmallestFovTo)
					flSmallestAngleTo = vAngleTo.y, flSmallestFovTo = flFOVTo;
			}
...
void CAntiAim::Run(CUserCmd* pCmd, bool* pSendPacket)
{
	...
	if (!Vars::AntiHack::AntiAim::Active.Value || G::ForceSendPacket || G::AvoidingBackstab || G::ShouldShift)
		return;
	...
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		if (!ShouldAntiAim(pLocal))
			return;
		if (bManualing)
			ManualMouseEvent(pCmd);
		else if (bSaved)
			{ I::EngineClient->SetViewAngles(vSavedAngles); bSaved = false; vSavedAngles = { 0, 0, 0 }; }
		...
		if (Vars::AntiHack::AntiAim::YawReal.Value == 7 || Vars::AntiHack::AntiAim::YawFake.Value == 7)
			bEdge = G::RealViewAngles.x == 89.f ? !FindEdge(flBaseYaw) : FindEdge(flBaseYaw);

		// Yaw
		if (!*pSendPacket)
			*pSendPacket = bSendState = false;
		else
		{
			if (bYawSet)
				*pSendPacket = bSendState = !bSendState;
			if (Vars::AntiHack::AntiAim::YawFake.Value == 0)
				*pSendPacket = bSendState = true;
		}

		if (bSendState && Vars::AntiHack::AntiAim::YawFake.Value)
		{
